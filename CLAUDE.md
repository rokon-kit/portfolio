...
4. **Accurate professional content only — but never delete an approved design element just because its content was false; keep the element and make its content honest (ADR-015).** Facts come from `docs/CONTENT.md`. Never invent or embellish experience, titles, metrics, latencies, versions, deployment status, outcomes, screenshots, or credentials. If a claim is unsourced, remove it or ask — do not soften it into plausible-sounding copy. Illustrative code/diagrams must be labelled illustrative. Do not confuse the owner with anyone else; contact details and identities are confirmed by the owner, not inferred.
...
- Parity checks: `node scripts/qa/prototype-parity.mjs --width 1440 --out <dir>` (serve `prototype/` with `python3 -m http.server 4173` from the repo root) diffs computed styles/geometry against the approved prototype. Read ADR-015 first: only its listed deliberate deviations are allowed.
- CSS pipeline gotcha: Tailwind's optimizer can drop the *standard* property when a vendor-prefixed twin follows it (it removed `backdrop-filter`). Write `-webkit-` first, standard last, and check the emitted CSS / computed style for critical properties.
